	...
	case WindowsMonitor:
		req.Monitor = &models.RunAction{
			...
			Args: []string{
				...
				`if ([string]::IsNullOrWhitespace($localIPAddress)) { Write-Output 'Cannot find a local IP address'; exit 2; };`,
				`$tcpClient = New-Object -TypeName System.Net.Sockets.TCPClient;`,
				`$connected = ($TcpClient.ConnectAsync($localIPAddress, $env:PORT)).AsyncWaitHandle.WaitOne(1000);`,
				`$tcpClient.Close();`,
				`if ($connected -eq $false) {Write-Output \"Port $($env:PORT) not ready yet.\"; exit 1; };`,
				`"`,
			},
			LogSource: "HEALTH",
			User:      params.User,
